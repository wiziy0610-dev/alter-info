"""
Alt-In 대체투자 뉴스 스크래퍼 (구글 뉴스 RSS 최종판)
------------------------------------------------------
...
def parse_rss(feed: dict) -> list[dict]:
    ...
    for item in soup.find_all("item")[:MAX_PER_FEED]:
        ...
        title_text = re.sub(r"\s*-\s*[^-]+$", "", title.get_text(strip=True))  # " - 출처명" 제거
        link_text  = link.get_text(strip=True) or (link.next_sibling or "").strip()
...
BODY_SELECTORS = [
    ...
    "div#article_body", "div.article_body", "div.news_end", "article",
]
...
def auto_summarize(title: str, body: str) -> list[str]:
    ...
    sents = [s.strip() for s in re.split(r"(?<=[.!?])\s+", body) if len(s.strip()) > 20 and "©" not in s]
    result = sents[:3]
    while len(result) < 3:
